...
void main() {
  ...
  for (int s = 0; s < numSeasons; s++) {
    ...
    for (final stats in controller.batterStats.values) {
      ...
      totalBB += stats.walks;
      totalHBP += stats.hitByPitch;
      totalK += stats.strikeouts;
      totalPA += stats.atBats + stats.walks + stats.hitByPitch; // 簡易PA（犠飛は無視）
    }
  }
  ...
  final hbpRate = totalHBP / totalPA;
  ...
  print('四球: $totalBB / 死球: $totalHBP / 三振: $totalK / 簡易PA: $totalPA');
  print('');
