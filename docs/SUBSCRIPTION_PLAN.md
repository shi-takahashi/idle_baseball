...
**サブスク管理・解約リンク（2026-05-31 追加）:**
- アプリ内から直接解約は**ストア仕様で不可**（課金 API に解約関数が無い）。代わりに Play の
  定期購入画面へ誘導するのが定石でストア審査でも実質必須。
- `BillingService.managementUrl()` が `CustomerInfo.managementURL`（アクティブなサブスクがある時のみ
  非 null）を返し、ショップ画面に「サブスクを管理・解約」リンクを表示 → `url_launcher` で外部に開く。
- 依存追加: `url_launcher: ^6.3.0`。`Entitlements` 変化に追従して URL を再取得（購入/復元/解約で出し入れ）。
...
### フェーズ 5: テスト（実課金なし）— ✅ 2026-06-04 完了

- [x] Play Console に**ライセンステスター**を登録 + 内部テストのテスター一覧に追加。
      versionCode 2 の課金コード入り AAB を内部テストにアップ → 実機インストール
- [x] **テスト購入が実機で成功**（2026-05-31）。RevenueCat 連携がエンドツーエンドで動作確認。
- [x] 残りのテスト項目を一通り確認（2026-06-04）:
  - [x] 3 サブスクそれぞれ購入 → 対応機能が解放される
  - [x] 解約 → 期限後に機能が戻る（テストでは更新間隔が短縮される）
  - [x] 再インストール後に購入が有効になる（同一 Google アカウントは起動時 `getCustomerInfo`
        で自動復元。「購入を復元」ボタンは審査要件 + 自動復元がこぼれた時のフォールバック）
  - [x] 時間スキップ購入中は**解禁通知が予約されない**（2026-05-29(2) の修正と整合）。
        併せて設定画面の通知トグルを「時間スキップ中は通知なし」表示 + 無効化に（2026-06-04, commit 7bc98ae）
  - [x] 広告削除購入中は試合前広告が出ない

**広告検証で必要だった暫定対応（2026-06-04, commit 7bc98ae）:**
- release ビルドは本番広告ユニットID が空文字だと広告が出ず「広告削除の効果」を観測できない。
  検証用に `ad_service.dart` の本番ユニットID に **Google テストID を暫定設定**した。
  **本番公開（フェーズ6）前に実 AdMob ID へ要差し替え**（広告ユニットID と Manifest の App ID の両方）。
- onboarding 中（1シーズン目の自チーム10試合未満）は仕様で広告が出ないため、検証は11試合目以降で行う。
- 内部テストは Play Console「反映済み」でも実機更新が遅れることがある。挙動が変わらない時は
  端末の実インストール versionCode を疑い、再インストールで確認する。
